# -*- coding: utf-8 -*-
"""
This package contains the lookup tables for the  zero (f_0) and first order terms (f_1) SAMOSA waveform model terms.
"""
...
class SAMOSAModelTermsTable(object):
    ...
    def from_package_luts(cls) -> "SAMOSAModelTermsTable":
        ...
        f0 = np.genfromtxt(__LUT_DIR__ / "LUT_F0.txt", **kwargs)
        f1 = np.genfromtxt(__LUT_DIR__ / "LUT_F1.txt", **kwargs)
    ...
    def _get_f0(self, xi: np.ndarray, z: np.ndarray) -> np.ndarray:
        ...
        f0 = self._set_fi_csi_eq_0(f0, xi, self.f0_csi_0)
    ...
    def _get_f1(self, xi: np.ndarray, z: np.ndarray) -> np.ndarray:
        ...
        f1 = self._set_fi_csi_eq_0(f1, xi, self.f1_csi_0)
    ...
    def _set_f0_z_idx_max_f0(
        ...
        f0[idx_max_f0] = 1. / 2. * self.sqrt_pi / z_idx_max_f0 ** (1. / 4.) * (
    ...
    def _set_f1_csi_gt_csi_max(
        ...
        f1[idx_max_f1] = (1. / 2.) * 1. / 4. * self.sqrt_pi / (z[idx_max_f1]) ** (3. / 4.)
        return f1
    ...
    @property
    def gamma_3_4(self) -> float:
        return 1.2254167024651779  # Gamma Function Value at 3/4
